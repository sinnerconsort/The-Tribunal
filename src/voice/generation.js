 ...
 * 
 * FIXES APPLIED:
 * - Replaced direct API calls with ST ConnectionManagerRequestService
 * - Added profile resolution helper functions
 * - Fallback to generateRaw if ConnectionManager unavailable
 * 
...
// ═══════════════════════════════════════════════════════════════
// ST CONNECTION MANAGER HELPERS
// ═══════════════════════════════════════════════════════════════

/**
 * Get ST context - handles various ways ST might expose it
 */
...
/**
 * Get connection profile ID by name
...
function getProfileIdByName(profileName) {
    ...
    if (!connectionManager) {
        console.warn('[The Tribunal] ConnectionManager not available');
        return null;
    }
    
    // "current" = currently active ST profile
    ...
    const profile = connectionManager.profiles?.find(p => 
        p.name === profileName || p.id === profileName
    );
...
// ═══════════════════════════════════════════════════════════════
// API CALLS - Using ST Connection Manager
// ═══════════════════════════════════════════════════════════════

/**
 * Make API call using ST's ConnectionManagerRequestService
 * Falls back to generateRaw if unavailable
 */
export async function callAPI(systemPrompt, userPrompt) {
    const ctx = getSTContext();
    
    // Get settings
    const maxTokens = extensionSettings.maxTokens || 600;
    const temperature = extensionSettings.temperature || 0.8;
    const profileSetting = extensionSettings.connectionProfile || 'current';
    
    // Try ConnectionManagerRequestService first
    if (ctx?.ConnectionManagerRequestService) {
        try {
            const profileId = getProfileIdByName(profileSetting);
            
            if (!profileId) {
                throw new Error(`Connection profile not found: ${profileSetting}`);
            }
            
            console.log('[The Tribunal] Using ConnectionManager profile:', profileSetting);
            ...
            // Build override payload for temperature/tokens
            ...
            console.warn('[The Tribunal] ConnectionManager request failed:', error);
            // Fall through to fallback
        }
    }
    
    // Fallback: Try generateRaw (uses main ST connection)
    if (ctx?.generateRaw) {
        console.log('[The Tribunal] Falling back to generateRaw');
        try {
            const combinedPrompt = `${systemPrompt}\n\n---\n\n${userPrompt}`;
            const response = await ctx.generateRaw(combinedPrompt, null, false, false, '', maxTokens);
            return response;
        } catch (error) {
    ...
    // Last resort: Check for legacy direct API settings
    const { apiEndpoint, apiKey, model } = extensionSettings;
    
    if (apiEndpoint && apiKey) {
        console.log('[The Tribunal] Using legacy direct API:', apiEndpoint);
        ...
        if (!response.ok) {
            ...
            throw new Error(`API ${response.status}: ${errorBody.substring(0, 200) || response.statusText}`);
        }
    ...
    throw new Error('No API connection available. Configure a connection profile in Settings.');
}
