...
async function onMessageReceived(messageId) {
    ...
    // Only runs if contacts autoDetect is enabled in settings
    // ═══════════════════════════════════════════════════════════════
    ...
    try {
        if (aiWillHandle && messageText) {
            ...
            if (aiExtractor) {
                // Only gather existing data for enabled extraction types
                const state = getChatState() || {};
                
                console.log('[Tribunal] Running AI extraction — enabled:',
                    [extractCases && 'cases', extractContacts && 'contacts',
                     extractEquipment && 'equipment', extractInventory && 'inventory']
                    .filter(Boolean).join(', ') || 'vitals/condition only');
                
                // Extract from message with only enabled types
                const results = await aiExtractor.extractFromMessage(messageText, {
                    existingCases: extractCases ? (state.ledger?.cases || []) : [],
                    existingContacts: extractContacts ? Object.values(state.contacts || {}) : [],
                    existingLocations: state.ledger?.locations || [],
                    existingEquipment: extractEquipment ? (state.equipment?.items || []) : [],
                    existingInventory: extractInventory ? (state.inventory?.carried || []) : [],
                ...
                if (results.error) {
                    ...
                    // Safety belt: strip any types the AI returned despite not being asked.
                    // The prompt now only includes enabled sections, but AI can hallucinate keys.
                    if (!extractContacts && results.contacts) {
                        results.contacts = { new: [], updated: [] };
                    }
                    if (!extractCases && results.cases) {
                        results.cases = { new: [], completed: [] };
                    }
                    if (!extractEquipment && results.equipment) {
                        results.equipment = { new: [], updated: [] };
                    }
                    if (!extractInventory && results.inventory) {
                        results.inventory = { gained: [], lost: [] };
                    }
...
export async function analyzeVoiceSentimentForNPCs(voiceResults, messageText) {
    ...
    // Respect contacts autoDetect setting
    const settings = getSettings();
    if (!settings?.contacts?.autoDetect) return;
