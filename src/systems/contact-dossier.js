...
import { getSkillPersonality, getProfileValue, getSkillName } from '../data/setting-profiles.js';
...
function buildDossierPrompt(contact, quipVoices) {
    const voicePersonalities = quipVoices.map(v => {
        ...
        return {
            id: v.voiceId,
            name: getSkillName(v.voiceId, skill?.signature || skill?.name || v.voiceId.toUpperCase()),
            stance: v.stance,
            score: v.score,
...
function parseDossierResponse(response, quipVoices) {
    ...
    for (const line of lines) {
        ...
        for (const voice of quipVoices) {
            ...
            const name = getSkillName(voice.voiceId, skill?.signature || skill?.name || voice.voiceId.toUpperCase());
