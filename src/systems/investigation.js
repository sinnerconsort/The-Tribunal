...
function createPanel() {
    ...
    panel.innerHTML = `
        ...
        <div style="${STYLES.masthead}">
            ...
            <div style="${STYLES.mastheadSub}">❦ ${getGenreSkillName('perception', 'PERCEPTION').toUpperCase()} SCAN ❦</div>
        </div>
...
function updateInvestigationFABVisibility() {
    // Visibility is managed by index.js updateFABVisibility() which uses
    // isInChatView() for robust page detection. This function is kept as a
    // no-op so existing callers don't error.
}

function setupFABVisibilityWatcher() {
    // FAB visibility is managed centrally by index.js updateFABVisibility()
    // which uses isInChatView() for robust page/drawer detection.
    // No local watcher needed.
}
...
async function generatePerceptionScan(sceneText) {
    ...
    const perceptionName = getGenreSkillName('perception', 'PERCEPTION');
    
    // Replace hardcoded PERCEPTION with genre-aware name in persona
    const genrePersona = tone.persona.replace(/You are PERCEPTION/i, `You are ${perceptionName.toUpperCase()}`);

    const systemPrompt = `${genrePersona}
${luckPrompt}
...
${perceptionName.toUpperCase()} CHECK: ${perceptionCheck.success ? 'SUCCESS' : 'FAILED'} (rolled ${perceptionCheck.total} vs ${perceptionDifficulty})
...
async function doInvestigate() {
    ...
    resultsEl.innerHTML = `<div style="${STYLES.loading}">
        ...
        <div style="font-size: 11px; letter-spacing: 2px;">${getGenreSkillName('perception', 'PERCEPTION').toUpperCase()} EXTENDING...</div>
    </div>`;
...
function showDiscoveryResults(investigation) {
    ...
    html += `<div style="padding: 8px 18px; background: rgba(0,0,0,0.03); border-bottom: 1px solid #c8b8a0; font-size: 10px; color: #5c4d3d; letter-spacing: 1px;">
        ${getGenreSkillName('perception', 'PERCEPTION').toUpperCase()} ${checkIcon} ${checkText} • ${investigation.weather.toUpperCase()}
    </div>`;
...
async function generateSkillReactions(discovery, sceneText) {
    ...
    for (const skillId of selectedSkillIds) {
        ...
        const genreName = getGenreSkillName(skillId, skill.signature);
        const systemPrompt = `You are ${genreName}. React to a specific discovered object.
...
async function handleDigDeeper(discoveryId) {
    ...
    try {
        ...
        for (const skillId of newSkillIds) {
            ...
            const genreName2 = getGenreSkillName(skillId, skill.signature);
            const systemPrompt = `You are ${genreName2}. React to a discovered object with fresh perspective.
