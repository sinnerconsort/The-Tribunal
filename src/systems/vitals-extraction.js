 ...
 * 
 * v2.1 - GENRE-AWARE MORALE
 * Loads per-genre keyword triggers from genre-vitals.js.
 * Genre keywords supplement (not replace) the universal base patterns.
 * 
 * Call processMessageVitals() on MESSAGE_RECEIVED to auto-update vitals.
 */

// ═══════════════════════════════════════════════════════════════
// GENRE-AWARE MORALE KEYWORDS
// ═══════════════════════════════════════════════════════════════

let _genreVitalsModule = null;
let _cachedGenreId = null;
let _cachedGenreDamagePatterns = [];
let _cachedGenreHealPatterns = [];

/**
 * Lazy-load genre vitals and build patterns for the active genre.
 * Caches compiled patterns until genre changes.
 */
function getGenreMoralePatterns() {
    // Lazy-load the module
    if (!_genreVitalsModule) {
        try {
            // Synchronous check — if the module was already imported, use it
            // Otherwise return empty (first message may miss genre keywords, not a big deal)
            import('../data/genre-vitals.js').then(m => {
                _genreVitalsModule = m;
                _cachedGenreId = null; // Force rebuild
                console.log('[Vitals] Genre vitals module loaded');
            }).catch(e => {
                console.warn('[Vitals] Genre vitals not available:', e.message);
            });
            return { damage: [], heal: [] };
        } catch (e) {
            return { damage: [], heal: [] };
    ...
    // Check if genre changed since last cache
    let activeGenreId = 'generic';
    try {
        const settings = window.TribunalState?.getSettings?.();
        // Settings store genre under two possible keys
        activeGenreId = settings?.genreProfile || settings?.activeProfile || 'generic';
    } catch (e) { /* use generic */ }
    
    if (activeGenreId !== _cachedGenreId) {
        const config = _genreVitalsModule.getGenreVitals(activeGenreId);
        _cachedGenreDamagePatterns = _genreVitalsModule.buildGenrePatterns(config.moraleDamageKeywords);
        _cachedGenreHealPatterns = _genreVitalsModule.buildGenrePatterns(config.moraleHealKeywords);
        _cachedGenreId = activeGenreId;
        console.log('[Vitals] Built genre patterns for', activeGenreId, 
            '— damage:', _cachedGenreDamagePatterns.length, 
            'heal:', _cachedGenreHealPatterns.length);
    }
    
    return { damage: _cachedGenreDamagePatterns, heal: _cachedGenreHealPatterns };
}
...
export function extractVitalsFromMessage(messageText) {
    ...
    // ═══════════════════════════════════════════════════════════
    // GENRE-SPECIFIC MORALE KEYWORDS (severity 1, soft signals)
    // These supplement the universal patterns above
    // ═══════════════════════════════════════════════════════════
    const genrePatterns = getGenreMoralePatterns();
    
    if (genrePatterns.damage.length > 0) {
        for (const pattern of genrePatterns.damage) {
            // Reset lastIndex for global patterns
            pattern.lastIndex = 0;
            const matches = messageText.matchAll(pattern);
            for (const match of matches) {
                moraleDelta -= 1;
                events.push(`Morale -1 (genre: ${match[0].trim().substring(0, 25)})`);
            }
        }
    }
    
    if (genrePatterns.heal.length > 0) {
        for (const pattern of genrePatterns.heal) {
            pattern.lastIndex = 0;
            const matches = messageText.matchAll(pattern);
            for (const match of matches) {
                moraleDelta += 1;
