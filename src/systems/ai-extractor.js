 ...
 * Only includes sections for enabled extraction types to save tokens
 * @param {string} messageText
 * @param {object} existingData
 * @param {string[]} excludeNames
 * @param {object} enabledTypes - Which types to extract { cases, contacts, equipment, inventory }
 */
function buildExtractionPrompt(messageText, existingData = {}, excludeNames = [], enabledTypes = {}) {
    const {
        cases = [],
        contacts = [],
        locations = [],
    ...
    // ── Existing data context (only for enabled types) ──────────
    const contextSections = [];
    
    if (enabledTypes.cases) {
        const titles = cases.map(c => `- "${c.title}" (${c.status})`).join('\n') || 'None';
        contextSections.push(`<existing_quests>\n${titles}\n</existing_quests>`);
    }
    if (enabledTypes.contacts) {
        const names = contacts.map(c => `- ${c.name}`).join('\n') || 'None';
        contextSections.push(`<existing_contacts>\n${names}\n</existing_contacts>`);
    }
    // Locations always included (no separate toggle, low token cost)
    const locNames = locations.map(l => `- ${l.name}${l.district ? ` (${l.district})` : ''}`).join('\n') || 'None';
    contextSections.push(`<existing_locations>\n${locNames}\n</existing_locations>`);
    
    if (enabledTypes.equipment) {
        const equip = equipment.map(e => `- ${e.name}`).join('\n') || 'None';
        contextSections.push(`<existing_equipment>\n${equip}\n</existing_equipment>`);
    }
    if (enabledTypes.inventory) {
        const inv = inventory.map(i => `- ${i.name}${i.quantity > 1 ? ` (x${i.quantity})` : ''}`).join('\n') || 'None';
        contextSections.push(`<existing_inventory>\n${inv}\n</existing_inventory>`);
    }
    
    // ── JSON response schema (only enabled types) ───────────────
    const jsonParts = [];
    
    if (enabledTypes.cases) {
        jsonParts.push(`  "quests": {
    ...
    // ── Rules (only for enabled types) ──────────────────────────
    const rules = [];
    
    if (enabledTypes.contacts) {
        rules.push(`- CONTACTS: Extract only NPCs (non-player characters) with actual proper names. Do NOT extract common words, descriptions, locations, or generic nouns as contacts. A contact must be a named CHARACTER (person, entity with a name). "${playerName}" is the PLAYER — NEVER add them as a contact.${excludeNames.length > 0 ? `\n- EXCLUDED NAMES (these are the player/main character, NOT NPCs - never extract these): ${excludeNames.join(', ')}` : ''}`);
    }
    if (enabledTypes.equipment) {
        rules.push(`- EQUIPMENT = wearable items ONLY: clothing, shoes, hats, glasses, jewelry, bags, watches, accessories
...
- EQUIPMENT changes apply to the PLAYER CHARACTER only — do NOT extract what NPCs are wearing`);
    ...
    if (enabledTypes.inventory) {
        rules.push(`- INVENTORY = anything a character picks up, receives, finds, buys, is given, uses, or would logically carry. Think of it as an invisible bag of holding — weapons, tools, consumables, food, drinks, documents, money, keys, phones, books, gifts, trinkets, souvenirs, anything tangible that isn't worn as clothing
...
- INVENTORY changes apply to the PLAYER CHARACTER only — items that NPCs carry, show, or use do NOT go into the player's inventory unless the player explicitly takes or receives them`);
    ...
    if (enabledTypes.cases) {
        rules.push(`- For completed quests, title must closely match existing`);
    }
    // Always-present rules
    rules.push(`- VITALS: health/morale changes for the PLAYER CHARACTER only. If an NPC gets hurt or emotionally affected, that is NOT a vitals change. Only report damage/healing that happens to "${playerName}"
...
- Only extract CLEAR, EXPLICIT changes - don't assume or infer`);
    ...
    // ── Assemble prompt ─────────────────────────────────────────
    const enabledList = [];
    if (enabledTypes.cases) enabledList.push('quests');
    if (enabledTypes.contacts) enabledList.push('contacts');
    enabledList.push('locations'); // always
    if (enabledTypes.equipment) enabledList.push('equipment');
    if (enabledTypes.inventory) enabledList.push('inventory');
    enabledList.push('vitals', 'condition'); // always
...
Extract the following types: ${enabledList.join(', ')}. Respond with ONLY valid JSON (no markdown, no explanation):
...
DO NOT EXTRACT: Hair, eyes, skin, body features, scars, tattoos, physical states, abstract descriptions, field labels, toys, stuffed animals, dolls, figurines, weapons, tools, food, drinks, documents, carried objects.
...
export async function extractFromMessage(messageText, options = {}) {
    const {
        ...
        existingInventory = [],
        excludeNames = [],
        enabledTypes = { cases: true, contacts: true, equipment: true, inventory: true },
        timeout = 30000
    } = options;
    ...
    try {
        ...
        }, excludeNames, enabledTypes);
        
        // Scale max_tokens based on how many types are enabled
        const typeCount = [enabledTypes.cases, enabledTypes.contacts, 
                          enabledTypes.equipment, enabledTypes.inventory].filter(Boolean).length;
        const maxTokens = Math.max(600, 400 + (typeCount * 250));
        ...
        const response = await makeAPICall(systemPrompt, prompt, maxTokens);
