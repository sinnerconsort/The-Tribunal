...
export function refreshLocations() {
    ...
    if (events.length === 0) {
        ...
        recentEvents.forEach((event, idx) => {
            ...
            html += `
                <div class="notebook-note ${event.aiGenerated ? 'notebook-note-ai' : ''}" data-index="${realIndex}">
...
export function initLocationHandlers() {
    ...
    // Expose globally so fire-and-forget memory generation can refresh the notebook
    window.tribunalRefreshLocations = refreshLocations;
