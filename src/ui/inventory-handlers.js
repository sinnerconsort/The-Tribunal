...
// Lazy-loaded genre skill name resolver
let _getSkillName = null;
import('../data/setting-profiles.js')
    .then(m => { _getSkillName = m.getSkillName || m.default?.getSkillName; })
    .catch(() => {});

function genreSkillName(skillId, fallback) {
    if (_getSkillName) {
 ...
 * Unlike removeInventoryItem which just splices, this fires an event
 */
export function trashItem(itemId) {
    ...
    window.dispatchEvent(new CustomEvent('tribunal:inventoryChanged', {
        detail: { action: 'trash', item: trashed }
    }));
...
function updateDetailPanel() {
    ...
    // Description: use description, fall back to context (investigation items)
    const descText = item.description || item.context || 'No description.';
    const descEl = document.getElementById('ie-detail-desc');
    
    // Check for [SKILL_NAME] tagged descriptions
    if (/\[([A-Z][A-Z_]+)\]/.test(descText)) {
        const parts = descText.split(/\[([A-Z][A-Z_]+)\]\s*/);
        let html = '';
        if (parts[0]?.trim()) html += parts[0].trim();
        for (let i = 1; i < parts.length; i += 2) {
            const skill = genreSkillName(parts[i].toLowerCase(), parts[i].replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()));
...
function formatSkillName(skill) {
    const fallback = skill.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    return genreSkillName(skill, fallback);
}
...
function initDetailPanelHandlers() {
    ...
    // Trash/discard button (two-tap confirm)
    let trashConfirmTimeout = null;
    let trashConfirmActive = false;
    ...
    document.getElementById('ie-trash-btn')?.addEventListener('click', () => {
        ...
        if (!trashConfirmActive) {
            // First tap — enter confirm mode
            const item = getInventoryState().items.find(i => i.id === selectedItemId);
            const name = item?.name || 'this item';
            trashBtn.textContent = `DISCARD ${name}?`;
            trashBtn.classList.add('inv-trash-confirming');
            trashConfirmActive = true;
            
            // Auto-reset after 3 seconds
            trashConfirmTimeout = setTimeout(() => {
                trashBtn.textContent = '✕ DISCARD';
                trashBtn.classList.remove('inv-trash-confirming');
                trashConfirmActive = false;
            }, 3000);
        } else {
            // Second tap — actually discard
            clearTimeout(trashConfirmTimeout);
            trashConfirmActive = false;
            
            const trashed = trashItem(selectedItemId);
            if (trashed) {
                toast(`Discarded: ${trashed.name}`, 'warning');
                selectedItemId = null;
                refreshDisplay();
            }
...
export async function scanForInventory() {
    ...
    try {
        ...
        if (!items || items.length === 0) {
            toast('No items found by AI', 'info');
            return;
        }
        
        toast(`AI found ${items.length} item(s), adding...`, 'info');
        
        // Ensure inventory structure exists on the chat state directly
        if (!chatState.inventory) {
            chatState.inventory = { carried: [], stash: {}, money: 0 };
        }
        if (!chatState.inventory.carried) {
            chatState.inventory.carried = [];
        }
        ...
        const existingNames = chatState.inventory.carried.map(i => i.name?.toLowerCase() || '');
        ...
        for (const item of items) {
            ...
            const isConsumableItem = ['cigarette', 'alcohol', 'drug', 'food', 'medicine', 'consumable'].includes(itemType);
            ...
            const newItem = {
                ...
                name: item.name,
                type: itemType,
                category: isConsumableItem ? 'consumable' : 'misc',
            ...
            chatState.inventory.carried.push(newItem);
        ...
        if (added > 0) {
            ...
            try {
                document.dispatchEvent(new CustomEvent('tribunal:inventoryChanged', {
                    detail: { source: 'scan', count: added }
                }));
            } catch (e) { /* silent */ }
 ...
 * NOTE: The main extraction pipeline is in events.js via aiExtractor.processExtractionResults
 * which handles inventory gains/losses. This hook is kept for any future direct inventory logic.
 */
async function onMessageReceived(messageIndex) {
    if (!autoExtractEnabled) return;
    
    try {
        // The main AI extraction pipeline (events.js) handles inventory extraction
        // via aiExtractor.extractFromMessage → processExtractionResults.
        // We just need to refresh the display after events.js finishes processing.
        // Use a small delay to let the extraction pipeline complete first.
        setTimeout(() => {
            refreshDisplay();
        }, 3000);
