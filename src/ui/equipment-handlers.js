...
async function scanForEquipment() {
    ...
    uiState.scanProgress = 'Analyzing persona...';
    refreshDisplay();
    ...
    try {
        ...
        uiState.scanProgress = `Found ${names.length} items, generating...`;
        refreshDisplay();
        ...
        for (let i = 0; i < names.length; i++) {
            const name = names[i];
            uiState.scanProgress = `Generating ${i + 1}/${names.length}: ${name}`;
            refreshDisplay();
...
function formatBonuses(bonuses) {
    ...
    return Object.entries(bonuses)
        .map(([s, m]) => `${m > 0 ? '+' : ''}${m} ${genreSkillName(s, s.replace(/_/g, ' '))}`)
        .join(', ');
}
...
function refreshDisplay() {
    ...
    // Show scanning overlay if active
    if (uiState.scanning) {
        container.innerHTML = `
            <div class="equip-scan-overlay">
                <div class="equip-scan-overlay-text"><span class="equip-scan-dot">●</span> SCANNING</div>
                <div class="equip-scan-overlay-sub">${uiState.scanProgress || 'Analyzing persona...'}</div>
            </div>
        `;
        return;
    }
...
function renderControls() {
    ...
    if (!uiState.addFormOpen) {
        const scanDisabled = uiState.scanning ? 'disabled style="opacity: 0.5; pointer-events: none;"' : '';
        div.innerHTML = `
            <button class="equip-ctrl-btn equip-scan" ${scanDisabled}><i class="fa-solid fa-magnifying-glass"></i> Scan Persona</button>
        ...
        const submit = async () => {
            ...
            uiState.addFormOpen = false;
            uiState.scanning = true;
            uiState.scanProgress = `Generating: ${name}...`;
            refreshDisplay();
